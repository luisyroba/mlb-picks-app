...
exports.fetchMlbMarketLines = fetchMlbMarketLines;
exports.normalizeMarketLines = normalizeMarketLines;
exports.mapEventMarketLinesToGameOdds = mapEventMarketLinesToGameOdds;
let cachedMarketLinesResponse = null;
...
function isHalfPointLine(line) {
    if (line === undefined || line === null)
        return false;
    const decimal = Math.abs(line % 1);
    return Math.abs(decimal - 0.5) < 0.001;
}
function isWholeOrHalfPointLine(line) {
    if (line === undefined || line === null)
        return false;
    const decimal = Math.abs(line % 1);
    return decimal < 0.001 || Math.abs(decimal - 0.5) < 0.001;
}
function getBookmakerSupportCount(byBookmaker) {
    const books = Object.values(byBookmaker ?? {}).filter((book) => isAvailable(book?.available));
    return Math.max(1, books.length);
}
function isReasonableMlbTotal(line, marketType) {
    if (line === undefined || line === null)
        return false;
...
function isReasonableRunLine(line, marketType) {
    if (line === undefined || line === null)
        return false;
...
function hasMonotonicOddsConflict(candidate, existing) {
    ...
    const sameFamily = existing.filter((line) => line.marketType === candidate.marketType &&
        line.side === candidate.side &&
        typeof line.line === 'number');
    for (const line of sameFamily) {
        if (candidate.line === undefined || line.line === undefined)
            continue;
        // OVER:
...
function pushUniqueLine(target, line) {
    ...
    const normalizedLine = {
        ...
        origin: Math.max(0, line.primarySupportCount ?? 0) > 0
            ? 'main'
            : (line.origin ?? 'alternate')
    };
    const key = makeLineKey(line.marketType, line.side, line.line);
    const existingIndex = target.findIndex((item) => makeLineKey(item.marketType, item.side, item.line) === key);
    if (existingIndex === -1) {
    ...
    const representative = nextDistance < currentDistance ? normalizedLine : current;
    target[existingIndex] = {
        ...representative,
        supportCount: Math.max(1, current.supportCount ?? 1) +
            Math.max(1, normalizedLine.supportCount ?? 1),
        primarySupportCount: Math.max(0, current.primarySupportCount ?? 0) +
            Math.max(0, normalizedLine.primarySupportCount ?? 0),
        origin: Math.max(Math.max(0, current.primarySupportCount ?? 0), Math.max(0, normalizedLine.primarySupportCount ?? 0)) > 0
            ? 'main'
            : (representative.origin ?? current.origin ?? normalizedLine.origin ?? 'alternate')
    };
}
function extractAltLines(odd, config) {
    const results = [];
    const byBookmaker = odd.byBookmaker ?? {};
    for (const book of Object.values(byBookmaker)) {
        if (!book?.altLines?.length)
            continue;
        for (const alt of book.altLines) {
            if (!isAvailable(alt.available))
                continue;
            const rawLine = config.lineField === 'spread' ? alt.spread : alt.overUnder;
            const parsedLine = toNumber(rawLine, Number.NaN);
            const parsedOdds = americanToDecimal(alt.odds);
            if (!Number.isFinite(parsedLine) || !parsedOdds)
                continue;
            if (config.lineField === 'overUnder') {
                if (!isReasonableMlbTotal(parsedLine, config.marketType))
                    continue;
            }
            if (config.lineField === 'spread') {
                if (!isReasonableRunLine(parsedLine, config.marketType))
                    continue;
            }
            results.push({
...
function getTotalPairsForMarket(lines, marketType) {
    const totals = lines.filter((line) => line.marketType === marketType &&
        (line.side === 'over' || line.side === 'under') &&
...
function getLineSupport(line) {
    return Math.max(1, line?.supportCount ?? 1);
}
function getLinePrimarySupport(line) {
    return Math.max(0, line?.primarySupportCount ?? 0);
}
function pickSupportedTotalPair(lines, marketType, options) {
    const completePairs = getTotalPairsForMarket(lines, marketType);
    const filteredPairs = options?.requirePrimaryPair
        ? completePairs.filter((pair) => getLinePrimarySupport(pair.over) > 0 &&
    ...
    const scoredPairs = filteredPairs.map((pair) => ({
        ...pair,
        primarySupport: getLinePrimarySupport(pair.over) + getLinePrimarySupport(pair.under),
        totalSupport: getLineSupport(pair.over) + getLineSupport(pair.under),
        balanceScore: Math.abs(pair.over.odds - 1.91) +
...
function pickAnchorTotalLine(lines, marketType) {
    return pickSupportedTotalPair(lines, marketType)?.line ?? null;
}
function getSpreadPairs(lines, marketType) {
    const spreads = lines.filter((line) => line.marketType === marketType &&
        (line.side === 'home' || line.side === 'away') &&
    ...
    for (const home of homeCandidates) {
        if (home.line === undefined)
            continue;
        for (const away of awayCandidates) {
            if (away.line === undefined)
                continue;
            if (Math.abs(Math.abs(home.line) - Math.abs(away.line)) > 0.001)
                continue;
            if (Math.sign(home.line) === Math.sign(away.line))
                continue;
            pairs.push({
...
function sanitizeTotalSide(lines, marketType, side, anchorLine) {
    const candidates = lines
        .filter((line) => line.marketType === marketType &&
        line.side === side &&
        typeof line.line === 'number' &&
        isWholeOrHalfPointLine(line.line))
    ...
    if (!candidates.length)
        return [];
    const anchor = candidates.find((candidate) => Math.abs((candidate.line ?? 0) - anchorLine) < 0.001) ??
        [...candidates].sort((left, right) => Math.abs((left.line ?? 0) - anchorLine) - Math.abs((right.line ?? 0) - anchorLine))[0];
    if (!anchor || anchor.line === undefined)
        return candidates;
    const anchorValue = anchor.line;
    const saferLines = side === 'over'
        ? [...candidates.filter((candidate) => (candidate.line ?? 0) < anchorValue)].sort((left, right) => (right.line ?? 0) - (left.line ?? 0))
        : [...candidates.filter((candidate) => (candidate.line ?? 0) > anchorValue)].sort((left, right) => (left.line ?? 0) - (right.line ?? 0));
    const riskierLines = side === 'over'
        ? [...candidates.filter((candidate) => (candidate.line ?? 0) > anchorValue)].sort((left, right) => (left.line ?? 0) - (right.line ?? 0))
        : [...candidates.filter((candidate) => (candidate.line ?? 0) < anchorValue)].sort((left, right) => (right.line ?? 0) - (left.line ?? 0));
    const kept = new Map();
...
function sanitizeTotalLadders(lines) {
    let sanitized = [...lines];
    for (const marketType of ['TOTAL', 'F5']) {
        const anchorLine = pickAnchorTotalLine(sanitized, marketType);
        if (anchorLine === null)
            continue;
        const nonTotals = sanitized.filter((line) => line.marketType !== marketType ||
            (line.side !== 'over' && line.side !== 'under'));
...
function sanitizeOddsRequestUrl(url) {
...
async function fetchJSON(url, timeoutMs) {
    ...
    try {
        ...
        if (!res.ok) {
            const error = new Error(`Odds API error ${res.status}`);
...
function buildEventsUrl(options, apiKey, cursor) {
...
function normalizeMarketLines(json) {
    ...
    return events.map((event) => {
        ...
        for (const odd of Object.values(oddsMap)) {
            ...
            // MONEYLINE
            if (betTypeID === 'ml') {
                const selectionSide = sideID === 'home' ? 'home' : sideID === 'away' ? 'away' : undefined;
                if (!selectionSide)
                    continue;
...
function pickMainMoneylinePair(lines, marketType = 'ML') {
    const homeCandidates = lines.filter((line) => line.marketType === marketType &&
        line.side === 'home' &&
        line.line === undefined);
    const awayCandidates = lines.filter((line) => line.marketType === marketType &&
        line.side === 'away' &&
        line.line === undefined);
    ...
    for (const home of homeCandidates) {
        for (const away of awayCandidates) {
            ...
            if (!impliedHome || !impliedAway)
                continue;
            pairs.push({
                homeML: home.odds,
                awayML: away.odds,
                balanceScore: Math.abs(overround - 1.05) + Math.abs(home.odds - away.odds) * 0.02
            });
        }
    }
    if (!pairs.length)
...
function pickMainRunLine(lines) {
    const candidates = getSpreadPairs(lines, 'RL').filter((pair) => getLinePrimarySupport(pair.home) > 0 &&
        getLinePrimarySupport(pair.away) > 0);
    if (!candidates.length)
        return undefined;
    const sorted = [...candidates].sort((a, b) => {
        const primarySupportA = getLinePrimarySupport(a.home) + getLinePrimarySupport(a.away);
        const primarySupportB = getLinePrimarySupport(b.home) + getLinePrimarySupport(b.away);
        ...
        const balanceScoreA = Math.abs(a.home.odds - 1.91) +
            Math.abs(a.away.odds - 1.91) +
            Math.abs(a.home.odds - a.away.odds) * 0.45;
        const balanceScoreB = Math.abs(b.home.odds - 1.91) +
...
function pickSharedEvaluableTotalPair(lines, marketType) {
    ...
    const nearbyPairs = getTotalPairsForMarket(lines, marketType)
        .filter((pair) => isHalfPointLine(pair.line) &&
    ...
    const preferredLine = Math.abs(mainBias) <= 0.0001
        ? null
        : mainBias > 0
            ? best.line + 0.5
            : best.line - 0.5;
    ...
    const preferredPairs = preferredLine === null
        ? []
        : nearbyPairs.filter((pair) => Math.abs(pair.line - preferredLine) < 0.001);
    const selected = sortPairs(preferredPairs.length ? preferredPairs : nearbyPairs)[0];
    if (!selected)
        return undefined;
    return {
        line: selected.line,
...
function pickMainTotalLine(lines) {
    return pickSharedEvaluableTotalPair(lines, 'TOTAL');
}
function pickMainF5Moneyline(lines) {
    return pickMainMoneylinePair(lines, 'F5');
}
function pickMainF5Total(lines) {
    return pickSharedEvaluableTotalPair(lines, 'F5');
}
function mapEventMarketLinesToGameOdds(marketEvent) {
    if (!marketEvent)
        return undefined;
