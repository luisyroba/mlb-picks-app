...
exports.buildGameDistribution = buildGameDistribution;
exports.impliedProbability = impliedProbability;
exports.expectedValue = expectedValue;
exports.fairOdds = fairOdds;
exports.probabilityMoneyline = probabilityMoneyline;
exports.probabilityRunLine = probabilityRunLine;
exports.probabilityTotal = probabilityTotal;
...
function average(values, fallback) {
    const active = values.filter((value) => typeof value === 'number' && Number.isFinite(value));
    if (!active.length)
        return fallback;
    return active.reduce((sum, value) => sum + value, 0) / active.length;
}
function weightedAverage(entries, fallback) {
    const active = entries.filter((entry) => typeof entry.value === 'number' && Number.isFinite(entry.value) && entry.weight > 0);
    if (!active.length)
        return fallback;
...
function blendOps(offense, opponentHand) {
    const splitOps = opponentHand === 'L'
        ? offense.vsLeftOps
        : opponentHand === 'R'
            ? offense.vsRightOps
            : undefined;
    const derivedOps = safeNumber(offense.onBasePct) !== undefined && safeNumber(offense.sluggingPct) !== undefined
...
function offenseIndex(offense, core, lineup, opponentStarterHand) {
    ...
    const obpFactor = clamp(1 + ((safeNumber(offense.onBasePct) ?? LEAGUE_OBP) - LEAGUE_OBP) / LEAGUE_OBP * 0.26, 0.92, 1.08);
    const slgFactor = clamp(1 + ((safeNumber(offense.sluggingPct) ?? LEAGUE_SLG) - LEAGUE_SLG) / LEAGUE_SLG * 0.28, 0.9, 1.1);
    const lineupFactor = clamp(1 +
        (safeNumber(lineup.lineupQualityAdjustment) ?? 0) / 100 * 0.18 -
        (safeNumber(lineup.missingKeyBatCount) ?? 0) * 0.025 -
        (lineup.catcherBackup ? 0.02 : 0), 0.82, 1.14);
    ...
    return clamp(runsFactor * 0.38 +
        opsFactor * 0.28 +
        lineupFactor * 0.14 +
        formFactor * 0.1 +
        obpFactor * 0.05 +
        slgFactor * 0.05 +
        teamFactor * 0.1, 0.74, 1.28);
}
function whipToRunsAllowed(whip) {
    if (safeNumber(whip) === undefined)
        return undefined;
    return clamp(4.15 + ((whip ?? 1.28) - 1.28) * 2.35, 2.6, 6.8);
}
function starterAdvancedMetricReliability(starter) {
    const innings = safeNumber(starter.inningsPitched) ?? 0;
    return clamp((innings - 12) / 38, 0.05, 1);
}
function starterRunsAllowed(starter) {
...
function bullpenRunsAllowed(bullpen) {
    const fatiguePenalty = (safeNumber(bullpen.fatigueScore) ?? 0) / 100 * 0.45 +
        (safeNumber(bullpen.last3DaysInnings) ?? 0) * 0.035 +
        (safeNumber(bullpen.last2DaysInnings) ?? 0) * 0.045;
    const availabilityBonus = (bullpen.closerAvailable === false ? 0.12 : 0) +
...
function sharedEnvironmentFactor(game) {
...
function getStarterInnings(starter) {
    return clamp(safeNumber(starter.expectedInnings) ?? DEFAULT_STARTER_INNINGS, 4.2, 6.8);
}
function fullGamePitchingFactor(starter, bullpen) {
...
function f5PitchingFactor(starter, bullpen) {
    const starterRa = starterRunsAllowed(starter);
    const bullpenRa = bullpenRunsAllowed(bullpen);
...
function deriveTotalsFromMeans(home, away) {
    return {
        total: home + away,
        margin: home - away
    };
}
function projectionBiasFromGameScript(layerA) {
    if (layerA.gameScript.scoringProjection === 'high')
        return 0.42;
    if (layerA.gameScript.scoringProjection === 'low')
        return -0.42;
    return 0;
}
function offenseEnvironmentBias(game, homeOffenseFactor, awayOffenseFactor) {
    ...
    const seasonComponent = clamp(((seasonRuns - LEAGUE_RUNS_PER_TEAM) / LEAGUE_RUNS_PER_TEAM) * 0.36, -0.18, 0.18);
    const recentComponent = clamp(((recentRuns - seasonRuns) / LEAGUE_RUNS_PER_TEAM) * 0.26, -0.12, 0.12);
    const factorComponent = clamp((averageOffenseFactor - 1) * 0.48, -0.16, 0.16);
    const opsComponent = clamp(((averageOps - LEAGUE_OPS) / LEAGUE_OPS) * 0.18, -0.08, 0.08);
    return clamp(seasonComponent + recentComponent + factorComponent + opsComponent, -0.24, 0.24);
}
function pitchingEnvironmentBias(game) {
    ...
    const starterComponent = clamp(((averageStarterRa - LEAGUE_STARTER_RA9) / LEAGUE_STARTER_RA9) * 0.26, -0.14, 0.14);
    const bullpenComponent = clamp(((averageBullpenRa - LEAGUE_BULLPEN_RA9) / LEAGUE_BULLPEN_RA9) * 0.18, -0.12, 0.12);
    return clamp(starterComponent + bullpenComponent, -0.22, 0.22);
}
function buildGameDistribution(game, layerA) {
    const homeOffense = offenseIndex(game.homeTeam.offense, game.homeTeam.core, game.homeTeam.lineupContext, game.awayTeam.starter.handedness);
    const awayOffense = offenseIndex(game.awayTeam.offense, game.awayTeam.core, game.awayTeam.lineupContext, game.homeTeam.starter.handedness);
    ...
    const rawHomeF5 = LEAGUE_RUNS_PER_TEAM * (5 / 9) * homeOffense * awayPitchingFactorF5 * envFactor +
    ...
    const totalBias = projectionBias +
        clamp(layerA.blockScores.parkWeather / 100 * 0.22, -0.22, 0.22) +
        totalsOffenseBias +
        totalsPitchingBias;
    const sideBias = clamp(layerA.pregameScore / 100 * 1.05, -1.05, 1.05) * 0.34 +
    ...
    const f5SideBias = clamp(layerA.blockScores.starter / 100 * 1.15, -1.15, 1.15) * 0.52 +
    ...
    const totalStdDev = clamp(2.28 + settled.total * 0.055 + avgBullpenFatigue / 100 * 0.18, 2.1, 3.3);
    const marginStdDev = clamp(2.02 + settled.total * 0.04 + Math.abs(settled.margin) * 0.035, 1.95, 3.05);
    const f5TotalStdDev = clamp(1.6 + settledF5.total * 0.048 + avgBullpenFatigue / 100 * 0.06, 1.55, 2.45);
    const f5MarginStdDev = clamp(1.38 + settledF5.total * 0.04 + Math.abs(settledF5.margin) * 0.03, 1.32, 2.2);
    return {
        muHome,
        muAway,
        muHomeF5,
...
function probabilityMoneyline(distribution, side, gameLength = 'full') {
...
function probabilityRunLine(distribution, side, line, gameLength = 'full') {
    const mean = gameLength === 'full' ? distribution.marginMean : distribution.f5MarginMean;
    const stdDev = gameLength === 'full' ? distribution.marginStdDev : distribution.f5MarginStdDev;
    const teamMargin = side === 'home' ? mean : -mean;
    const target = -(line ?? 0);
...
function probabilityTotal(distribution, side, line, gameLength = 'full') {
